...
// --- Helpers (sin cambios, los mantuve como estaban) ---
...
// Badge de estado con colores y estilos consistentes
function StatusBadge({ status }) {
  const statusKey = (status || 'desconocido')
    .toLowerCase()
    .normalize('NFD')
...
// Un avatar simple para el cliente
...
export default function PagosLista() {
  ...
  const FilterButton = ({ status, label, count }) => (
    <button
      className={`pl-filter-btn ${filter === status ? 'pl-filter-btn--active' : ''}`}
    ...
    <div className="pl-container">
      ...
      <header className="pl-header">
        ...
        <div className="pl-header-actions">
            <button className="pl-action-btn" onClick={() => navigate('/pago/nuevo')}>
      ...
      <main className="pl-main-card">
        ...
        <div className="pl-table-wrapper">
          {loading ? (
            <div className="pl-state-view">Cargando pagos...</div>
          ) : error ? (
            <div className="pl-state-view pl-state-view--error">{error}</div>
          ) : filteredPagos.length === 0 ? (
            <div className="pl-state-view">No hay pagos que coincidan con el filtro.</div>
          ) : (
            <table className="pl-table">
              ...
              <tbody>
                {filteredPagos.map((p) => (
                  <tr key={p.id}>
                    <td>
                      <span className="pl-id-cell">#{p.id}</span>
                    </td>
                    <td>
                      <CustomerAvatar name={p.cliente} />
                    </td>
                    <td>
                      <span className="pl-amount-cell">{money(p.monto, p.moneda)}</span>
                    </td>
                    <td>
                      <StatusBadge status={p.estado} />
                    </td>
                    <td>
                      <span className="pl-date-cell">{fechaHora(p.fecha)}</span>
                    </td>
                    <td>
